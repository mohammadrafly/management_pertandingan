...
<form action="{{ route('manajer.atlet.update', $data->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-4 flex gap-5">
        <div class="w-full">
            ...
            <input type="text" name="tempat" id="tempat" class="w-full px-3 py-2 border rounded @error('tempat') border-red-500 @enderror" value="{{ old('tempat', $tempat) }}">
            @error('tempat')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="w-full">
            ...
            <input type="date" name="tanggal" id="tanggal" class="w-full px-3 py-2 border rounded @error('tanggal') border-red-500 @enderror" value="{{ old('tanggal', $tanggal) }}">
            @error('tanggal')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="mb-4">
        ...
        @enderror
        @if($data->foto)
            <div class="py-2 flex font-thin text-sm">
                <label for="currentSuratTugas">Foto saat ini: </label>
                <a href="{{ asset('storage/fotos/'.$data->foto) }}" download class="text-blue-500 hover:text-blue-600">{{ $data->foto }}</a>
            </div>
        @endif
    </div>
    ...
    <div class="mb-4">
        ...
        @if($data->foto_ktp)
            <div class="py-2 flex font-thin text-sm">
                <label for="currentSuratTugas">Foto KTP saat ini: </label>
                <a href="{{ asset('storage/foto_ktp/'.$data->foto_ktp) }}" download class="text-blue-500 hover:text-blue-600">{{ $data->foto_ktp }}</a>
            </div>
        @endif
    </div>
    ...
    <div class="mb-4">
        ...
        @error('ijazah_karate')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
        @enderror
        @if($data->ijazah_karate)
            <div class="py-2 flex font-thin text-sm">
                <label for="currentSuratTugas">Ijazah Karate saat ini: </label>
                <a href="{{ asset('storage/ijazah_karate/'.$data->ijazah_karate) }}" download class="text-blue-500 hover:text-blue-600">{{ $data->ijazah_karate }}</a>
            </div>
        @endif
    </div>
