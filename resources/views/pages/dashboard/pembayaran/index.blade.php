...
<table id="table" class="w-full display compact">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ $item->id_pertandingan }}</td>
            <td>{{ $item->id_tim }}</td>
            <td>{{ 'IDR ' . number_format($item->total, 0, ',', '.') }}</td>
            <td>{{ $item->status}}</td>
            <td class="flex items-center">
                {{-- <a href="{{ route('pembayaran.update', $item->id)}}" class="transition-colors duration-300 bg-blue-500 p-1 rounded text-white mr-3 hover:bg-white border-blue-500 border hover:text-blue-500">
